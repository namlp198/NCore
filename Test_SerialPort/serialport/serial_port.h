...
#define REGISTRY_MAX_KEY_LENGTH 255 
        ...
        */
        ...
        static int processRegistryValue(HKEY rootKey, std::string searchKey, ValueProcess func, bool searchSubKey = true)
        {
            ...
            // Create registry key
            HKEY hKey;
            if (RegOpenKeyEx(rootKey, (LPCWSTR)searchKey.c_str(), 0, KEY_READ, &hKey) == ERROR_SUCCESS)
            {
...
#pragma warning(suppress : 26451)
                    ...
                    for (int i = 0; i < (int)numOfValue; i++)
                    {
                        ...
                        // Get the subkey
                        if (RegEnumValue(hKey, i, (LPWSTR)valueNameBuffer.get(), &valueNameBufferLen, NULL, &dataType, dataByteBuffer.get(), &sizeOfDataByteBuffer) == ERROR_SUCCESS)
                        {
                    ...
                    for (int i = 0; i < (int)numOfSubKey; i++)
                    {
                        ...
                        if (RegEnumKeyEx(hKey, i, subKey, &subKeyBufferLen, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
                        {
                            ...
                            try
                            {
                                ...
                                CT2CA pszConvertedAnsiString(subKey);
                                // construct a std::string using the LPCSTR input
                                std::string sSubKey(pszConvertedAnsiString);
                                result += processRegistryValue(rootKey, searchKey + "\\" + sSubKey, func, searchSubKey);
                            }
                            catch (...)
                            {
                                // Close
