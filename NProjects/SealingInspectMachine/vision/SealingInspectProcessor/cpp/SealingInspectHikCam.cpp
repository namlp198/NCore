...
CSealingInspectHikCam::CSealingInspectHikCam(ISealingInspectHikCamToParent* pInterface)
{
	...
	for (int i = 0; i < MAX_NUMBER_OF_CAMERA_INSPECT; i++) {
		m_pCamera[i] = NULL;
...
{
	CFrameGrabberParam grabberParam[MAX_NUMBER_OF_CAMERA_INSPECT];
	...
	grabberParam[0].SetParam_FrameCount(MAX_NUMBER_OF_FRAME_TOPCAM);
	...
	grabberParam[1].SetParam_FrameCount(MAX_NUMBER_OF_FRAME_TOPCAM);
	...
	grabberParam[2].SetParam_FrameCount(MAX_NUMBER_OF_FRAME_SIDECAM);
	...
	grabberParam[3].SetParam_FrameCount(MAX_NUMBER_OF_FRAME_SIDECAM);

	int nCamIdx = 0;
	for (nCamIdx = 0; nCamIdx < MAX_NUMBER_OF_CAMERA_INSPECT / 2; nCamIdx++)
	{
		// Buffer
		...
		DWORD dwFrameCount_TopCam = MAX_NUMBER_OF_FRAME_TOPCAM;
	...
	for (nCamIdx = 2; nCamIdx < MAX_NUMBER_OF_CAMERA_INSPECT; nCamIdx++)
	{
		// Buffer
		if (m_pCameraImageBuffer[nCamIdx] != NULL)
		{
		...
		DWORD dwFrameCount_SideCam = MAX_NUMBER_OF_FRAME_SIDECAM;
...
{
	if (nCamIdx < 0 || MAX_NUMBER_OF_CAMERA_INSPECT <= nCamIdx)
		return NULL;
...
{
	if (nGrabberIndex < 0 || MAX_NUMBER_OF_CAMERA_INSPECT <= nGrabberIndex)
		return -1;
	...
	m_pCameraImageBuffer[nGrabberIndex]->SetFrameImage(nCurrentFrameIdx, (LPBYTE)pBuffer);
	
	// Push Inspect Wait List
	CSingleLock localLock_Wait(&m_csInspectWaitList[nGrabberIndex]);
	localLock_Wait.Lock();
	m_queueInspectWaitList[nGrabberIndex].push(nCurrentFrameIdx);
	localLock_Wait.Unlock();

	int nNextFrameIdx = nCurrentFrameIdx + 1;

	if (nGrabberIndex < 2)
		nNextFrameIdx = nNextFrameIdx % MAX_NUMBER_OF_FRAME_TOPCAM;
	else
		nNextFrameIdx = nNextFrameIdx % MAX_NUMBER_OF_FRAME_SIDECAM;
...
int CSealingInspectHikCam::SingleGrab(int nCamIdx)
{
	if (nCamIdx < 0 || MAX_NUMBER_OF_CAMERA_INSPECT <= nCamIdx)
		return 0;
