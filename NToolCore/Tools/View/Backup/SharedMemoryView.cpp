...
void CSharedMemoryView::DrawInspectLine(CDC &dc)
{
	...
	for(int nFrameIndex = nStartFrameIndex; nFrameIndex <= nEndFrameIndex; nFrameIndex++)
	{
		...
		int nChipCrackDrawLine = (int)((nFindFrameSideLine - rtViewImageArea.left + m_pEdgeInspectStatus->GetChipCrackRange()) * m_dZoomLevel);
		int nBurrDrawLine = (int)((nFindFrameSideLine - rtViewImageArea.left + m_pEdgeInspectStatus->GetBurrRange()) * m_dZoomLevel);
		int nInChipCrackDrawLine = (int)((nFindFrameSideLine - rtViewImageArea.left + m_pEdgeInspectStatus->GetInChipCrackRange()) * m_dZoomLevel);
		int nInBurrDrawLine = (int)((nFindFrameSideLine - rtViewImageArea.left + m_pEdgeInspectStatus->GetInBurrRange()) * m_dZoomLevel);
		...
		dc.MoveTo(nChipCrackDrawLine, nDrawLineStartY);
		dc.LineTo(nChipCrackDrawLine, nDrawLineEndY);
		dc.SelectObject(pOldpen);
		...
		pOldpen = dc.SelectObject(&ChippingLinePen);
		dc.MoveTo(nInChipCrackDrawLine, nDrawLineStartY);
		dc.LineTo(nInChipCrackDrawLine, nDrawLineEndY);
		dc.SelectObject(pOldpen);
...
void CSharedMemoryView::DrawDefect(CDC &dc)
{
	if(m_pEdgeInspectStatus == NULL)
		return;
	...
	for(int i=0; i<(int) (*vecDefectList).size(); i++)
	{
		...
		// if(pDefect->m_emDefectType != emDefectType_Chipping)
		//  continue;
